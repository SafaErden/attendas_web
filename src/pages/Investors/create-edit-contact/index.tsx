...
import { ICreateEditContactProps } from '../investors'
...
export const CreateEditContact: NamedExoticComponent<ICreateEditContactProps> = memo(() => {
	...
	const { selectedContact, contacts, selectedInvestor } = useSelector<IGlobalState, IInvestorsState>(
	...
	const handleSubmit = (values: Partial<IContact>) => {
		if (selectedInvestor?.investorId) {
			const selectedContactIndex = selectedInvestor?.contacts?.findIndex(
				(c) => (c.contactId ?? c.tempId) === (selectedContact?.contactId ?? selectedContact?.tempId),
			)
			let newContacts = [...(selectedInvestor?.contacts || [])]
			...
			dispatch(
				GenericActionCreator<IPutInvestorContacstAction>({

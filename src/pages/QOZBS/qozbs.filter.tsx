...
import { ISetFilterAction, QOZBS_ACTION_TYPES } from './qozbs.actions'
import { IGlobalState } from '../../redux/reducers'
import { useDebounce } from '../../hooks/use-debounce'
import { ISetPageAction, PAGINATION_ACTION_TYPES } from '../../shared/pagination/pagination.actions'

export const QozbsFilter: NamedExoticComponent<any> = memo(() => {
	const dispatch = useDispatch()

	const [searchText, setSearchText] = useState('')

	const { filter, locations } = useSelector((state: IGlobalState) => state.qozbs)
	...
	const handleChange: React.ChangeEventHandler<HTMLInputElement | HTMLSelectElement> = (e) => {
		...
		dispatch(
			GenericActionCreator<ISetFilterAction>({
				...
				data: {
					name: e.target.name,
					data: e.target.name === 'location' ? locations.find((l) => l.state === e.target.value) : e.target.value,
				},
			}),
		)
	}
		...
		<div className='flex-wrap flex flex-col sm:flex-row justify-between sm:items-center gap-2 mt-3 pt-5 border-t border-gray-200'>
			...
			<div className='flex-wrap flex-col flex sm:items-center sm:flex-row gap-2'>
				...
				<select
					...
					value={filter.location?.state ?? 'Select a role...'}
				>

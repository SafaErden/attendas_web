...
import { FUNDS_ACTION_TYPES, ISetFundFilterAction } from './funds.actions'
import { ISetPageAction, PAGINATION_ACTION_TYPES } from '../../shared/pagination/pagination.actions'

export const FundsFilter: NamedExoticComponent<any> = memo(() => {
	const dispatch = useDispatch()

	const [searchText, setSearchText] = useState('')

	const { filter, entityTypes, locations } = useSelector((state: IGlobalState) => state.funds)
	...
	useEffect(() => {
		...
		dispatch(
			GenericActionCreator<ISetFundFilterAction>({
	...
	const handleEntityChange: React.ChangeEventHandler<HTMLInputElement | HTMLSelectElement> = (e) => {
		dispatch(
			GenericActionCreator<ISetPageAction>({
				type: PAGINATION_ACTION_TYPES.SET_CURRENT_PAGE,
				data: {
		...
		dispatch(
			GenericActionCreator<ISetFundFilterAction>({
				...
				data: {
					name: e.target.name,
					data: entityTypes.find((ent) => ent.entityTypeName === e.target.value),
				},
			}),
		)
	}
			...
			<div className='flex gap-2 flex-col sm:flex-row'>
				...
				<div className='flex-wrap flex-col flex sm:items-center sm:flex-row gap-2'>
					...
					<select
						...
						value={filter.type?.entityTypeName}
					>
						<option>All Entities</option>
						{entityTypes.map((ent) => (
							<option key={ent.entityTypeId} value={ent.entityTypeName}>
								{ent.entityTypeName}
							</option>
						))}
					</select>
